...
As we move beyond the initial Generative AI POCs and ChatBots towards production ready AI systems, [AI Agents and Agentic AI](https://www.youtube.com/watch?v=KrRD7r7y7NY) are emerging as the driving force behind intelligent automation in enterprises. These agents, capable of collaborating with each other, understanding complex tasks, retrieving information, and making informed decisions, are transforming how teams operate.

In this demo, we’ll explore how to build and fully operationalize AI Agents with Red Hat OpenShift, Red Hat Developer Hub, and OpenShift AI using a Stock Recommendation use case. We are also going to demonstrate how to evaluate and guard AI systems with scale using technologies such as [Trusty AI](https://github.com/trustyai-explainability) and managed cloud services.

Responsible AI is an integral theme in our demo as we require the AI Agent to adhere to safety and regulatory rules. In addition, we would like to evaluate and observe the model performance and accuracy at all stages of the AI system. This is extremely important in highly regulated industries such as fintech.

In addition, we are going to demonstrate how such agents can easily utilize Generative AI services of public cloud and SaaS platforms such as Amazon’s AWS Bedrock, MS Azure AI Foundry, and Google Vertex AI, using integrations offered by Agentic frameworks such as LangGraph.

Through this blog, we’ll use the example of a fictitious large corporation’s fintech team responsible for investment and portfolio management. After several successful PoCs and early initiatives with Generative AI chatbots, the team is now exploring a more autonomous approach that will utilize the latest advancements in AI systems. Their goal is to build an AI system that can automate parts of their stock recommendation process with minimal user guidance.


## Stock recommendation AI Agents

The team’s initial high level architecture for the AI system is depicted below (during the demo walk-through we will break it down into its logical parts).
...
For grounding their AI Agent with relevant financial context (e.g., the financial reports of public companies), they plan to use **RAG (Retrieval-Augmented Generation)**.
The company’s data platform services are distributed across multiple public cloud platforms and it is decided that the AI Agent will integrate with these services.
...
At the base level, an OpenShift cluster (2.17 or newer) should be installed on-prem or on the cloud.
- The worker nodes should have a GPU that supports the inference model you are planning to set the demo with. 
...
The demo also requires these key OpenShift applications to be installed:
...
To satisfy this large requirement we created an automation in the form of a “Red Hat validated pattern”. The validated pattern can easily be applied to your OpenShift cluster and take care of all these requirements.
Follow these detailed instructions to install the pattern: [Infrastructure Deployment for Stock recommendation AI Agents and Hyperscalers](../26/stock-ai-agents-installation).

Note each of these applications has lower level dependencies (e.g. Autorino, NFD, Service Mesh etc.), which are automatically handled by the validated pattern setup. If you prefer to set up these components manually, you can refer to the exact requirements in this [post](../26/stock-ai-agents-installation).
...
If you can satisfy the demo requirements yourself without the validated pattern, you can import the  template to developer Hub directly from [here](https://github.com/luis5tb/developerhub-agentic-demo).
In case you would like to change the template, you can fork the repository and import your fork.

#### Provisioning the Demo AI Agent resources using the Template

Login to Developer Hub and locate the template by navigating to the Create side menu.
Look for a template named: **“Stock recommendation AI Agents with OpenShift, OpenShift AI and Bedrock”**,  click choose and follow the setup screens.

![RHDH Template](../../../../images/stock-ai-agents/rhdh_template.png?w=1024)

You will need to provide the following mandatory inputs during the templates setup:
...
- Component deployment information:
  ...
  - **ArgoCD Namespace**: Set to `multicloud-gitops-hub` by the ValidatedPattern. If manually deployed, adapt it to the one you configured.
...
- Model Information:
  ...
  - **Model Location**: Choose between S3 and OCI options:
    ...
    - **OCI**:
      - **UCI URI**: The model container image URI, starting with `oci://`.
      - [Optional] **Registry secret**: Name of the secret containing Registry credentials in case that is needed for fetching the model. The secret can be created using:
        ```bash
...
To execute the **LLM evaluation** using the Pipeline created by the Develope Hub template, follow these steps:
...
In addition to pre-deployment evaluations, we plan to introduce **runtime guardrails** to act as a **safety gateway** between the agents and the LLM. These guardrails will be inspired by the ones defined by **Hyperscalers**.


### Using the AI Agent for Stock Recommendation and Analysis

In the Developer Hub Catalog, you can check the topology of the template deployed and view the Agent deployed. Click on the deployed agent to interact with it.
...
Even if this appears as a single Pod in OpenShift, internally, it consists of multiple **specialized agents** working together. The architecture is as follows:
...
4. **Upload documents**: You need to precreate a new S3 bucket for the Knowledge Base, and upload the documents you want to use as the Knowledge Base. Then select them in the form. For this demo, we added a couple of files with fake data about 2 companies. You can see the data uploaded [here](https://github.com/luis5tb/developerhub-agentic-demo/tree/main/skeleton/fake_reports). And see that the reply from the agent actually includes the data from the uploaded files.
...
To use the Bedrock Knowledge Base in your agent/application, you must configure the necessary **credentials **in the secret. For the Validated Pattern, configure the credentials in `values-secret.yaml.template` file with the correct AWS Knowledge Base credentials and reload the secrets:
...
Similarly, **Google Vertex AI Search Service** is also supported. To use it, change the `VECTORDB_PROVIDER` environment variable to `GOOGLE` and set the correct VertexAzure AI Search Service credentials. For the Validated Pattern, update the `values-secret.yaml.template` file with the correct Vertex AI Search Service credentials and reload the secrets:
...
Note you will need to download your **service account** `API KEY`, in json format, and point the `GOOGLE_API_KEY` to the file.

The steps to create the data store and make it available are the next:
1. Create a **bucket** at **Cloud Storage** and upload the desired data.
2. At the **Agent Builder**, create a **Data Store**, selecting the data source, in this case Cloud Storage.
   * Point the **Data Store** to the Cloud Storage data.
3. At the **Agent Builder**, create an **App** pointing to the created **Data Store**.
   * Turn Enterprise edition on at the **Advanced Configuration** tab to allow retrieval.
...
1. **Builds** a new Agent Service image (see next Section for more details).
...
The template is deploying resources in OpenShift under two different OpenShift namespaces:
- One for the ML backend resources that are mostly owned by OpenShift AI.
- One for the AI application side resources atop OpenShift.

The two namespaces are based on the `Namespace` input variable that is provided during the Developer Hub template run. In our example, we use `fintech-team` as the namespace parameter. These two namespaces are created: `vllm-fintech-team` and `fintech-team`.

#### What is inside the namespaces?

##### vllm-fintech-team

Namespace `vllm-fintech-team` contains these ML resources in OpenShift and in OpenShift AI:
...
The Git account used is based on the `GitHub Organization` input variable that is provided during the developer Hub template run.

In our example the template provisions these two git reposotories:
- `https://github.com<the team’s git account>/fintech-team-agent-hyperscalers`:
This repo contains the **agent python code** that is pulled by the Tekton Pipleline to build the Agent image. This means that even after the demo is deployed, you can evolve and edit the project directly on your Git account. The template is also installing a **webhook** to track commits and trigger the application build Pipeline in OpenShift automatically upon code changes to the agent repository. This way you can keep your AI Agent service updated with the latest code changes.
...
The template is also utilizing secret management in OpenShift (facilitated by [Vault](https://developer.hashicorp.com/vault/docs/platform/k8s/vso) kubernetes operator) to retrieve the secrets that were previously created and persisted  by the validated pattern. In case you did not use the Validated Pattern to install the OpenShift demo dependencies, you can create and persist these secrets yourself to OpenShift using Vault or other secret manager.

The next is the list of needed secrets in the Vault:
- **Environment Setup**:
  ...
  - **modelregistry-keys**: values for the database configuration that the model registry use.
- **RHDH Template**:
...
Note the Vault is also being used by the template to copy secrets between namespaces, such as to get access to the Inference Service Token from the Agent namespace. If you use another secret store the secretStore can be configured on the templates by adapting the `values.yaml` at the helm templates, for example [here](https://github.com/luis5tb/developerhub-agentic-demo/blob/2f31969ff116544ce137ba624b566209fbd79217/manifests/helm/agent/values.yaml#L42).

In addition, after the template deployment is triggered another secret is required on the VLLM namespace (not in the Vault):
- **registry-keys**: The `.dockerconfigjson` with the credentials to fetch images (ModelCar containers) from the registry. Required when OCI is used to serve the models, instead of S3. The name is passed on the template and it can be created with:
    ```bash
...
- For the Agentic framework, we plan to complement LangGraph with [LlamaStack](https://github.com/meta-llama/llama-stack) — a collection of standard and interoperable APIs designed to support the core building blocks required to bring generative AI applications to market (think of it as the Gen AI middleware APIs). These building blocks are delivered as interoperable APIs, enabling a wide range of Service Providers to offer their own implementations. This approach allows AI developers to switch seamlessly between compute environments and AI and data platform providers (such as Red Hat AI, AWS Bedrock, Azure, Databricks, and Ollama) with minimal adjustments to configurations. As one example, with a Red Hat AI distribution of the Llama Stack (Llama Stack HTTP server on OpenShift) the agent application will leverage standard Llama Stack Inference API calls with the [vLLM Provider](https://blog.vllm.ai/2025/01/27/intro-to-llama-stack-with-vllm.html) instead of relying on proprietary APIs like OpenAI.
- We plan to complement the evaluation part of the Responsible AI with guardrails protecting the runtime application. Options are Trusty AI guardrails with a Granite guard model or Bedrock/Azure Foundry managed guardrails service.
...
For simplicity the Validated Pattern is allowing you to set Developer Hub with personal GitHub authentication. In case you took this approach, please note that only one git user can use the demo parts on Developer Hub. 
...
When running the AI template, sometimes the `PipelineRun` (in charge of triggering the image build pipeline of the AI Agent) fails due to a race condition in Argo CD.
...
By default TrustyAI is blocking remote code execution. There is not a supported way of configuring this through the operator, so there is a need for a manual workaround by editing the ConfigMap `trustyai-service-operator-config` at `redhat-ods-applications` namespace:
- Add the next annotation and save it: `opendatahub.io/managed: 'false'`
...
When deploying the model from a container registry you need to pass the OCI uri on the Red Hat DeveloperHub template and optionally a pointer to a secret containing the credentials to pull the model image from that registry. As the VLLM namespace is created during the template deployment phase, creating that secret is left as a manual action after instancing the template. The reason behind this is to allow easy configuration of different registries for different models.
